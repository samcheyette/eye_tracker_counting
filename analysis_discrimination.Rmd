...
data.resp.avs <- data.resp.loc %>%
                 ...
                 # mutate(within_x2 =  arr2 * sum((2/(1 + exp(-2 -0.5*belowX)) - 1))) %>%
                  mutate(within_x1 =  sum(arr1 * (2/(1 + exp(-2 -0.5*belowX))))) %>%
                  mutate(within_x2 =   sum(arr2 * (2/(1 + exp(-2 -0.5*belowX))))) %>%
                  top_n(n=1, wt=id)%>%
                  group_by(trial_id) %>%
...
data.resp.avs.subs$Dots_Shown2 -  data.resp.avs.subs$within_x2
...
ggplot(data=data.resp.avs.subs) + 
      geom_point(aes(Dots_Shown1, within_x1,color="1")) + 
   geom_point(aes(Dots_Shown2, within_x2, color="2"))  +
  stat_smooth(aes(Dots_Shown1, within_x1,color="1")) +
  stat_smooth(aes(Dots_Shown2, within_x2, color="2")) +
  geom_abline()
