...
ggplot(data=data.resp, aes(x=Dots_Shown2/(Dots_Shown2, y=Dots_Counted)) +
        stat_summary_bin(fun.data="mean_se", bins=11) +
             ...
             # probit_smooth(color="black", se=TRUE, alpha=0.3)+
              ...
              scale_color_manual(values=c("blue", "red")) +
              ylab("p(choose 2)") + #xlab("Numerical Ratio") +
              xlab(expression(paste("Numerical ratio  ", (N[1] / N[2])))) +
             ylim(0,1)# +
            #facet_wrap(~I(Time1==Time2))
...
ggplot(data=data.resp.vary, aes(x=Dots_Shown2/(Dots_Shown1 + Dots_Shown2), y=Dots_Counted, color=cond)) +
