...
ggplot(data=data.resp.avs.subs, aes(x=cond_ord, y=Dots_Counted)) +
          stat_summary(fun.y="mean", geom="point", alpha=0.75, size=4.) +
...
ggsave("figs/dots_counted_diff.pdf", width=6,height=5)
...
ggplot(data=data.resp.avs.subs, aes(x=cond_ord, y=within_x_log_diff)) +
        ...
        paper_theme_3 + 
         ...
         # theme(legend.title=element_blank(), legend.text=element_blank()) +
            ...
            xlab("")  + ylab("Difference in dots seen (2 vs. 1)")

ggsave("figs/dots_seen_diff.pdf", width=6,height=5)
