...
ggplot(data=data.resp, aes(x=Time, y=100*pct_err, fill=Time)) +
     # geom_violin(color="blue", alpha=0.75) +
      stat_summary(geom="bar", alpha=0.8, fun.y="mean") +
      stat_summary(geom="errorbar", width=0.25,size=1.0, fun.data="mean_se") +
      stat_summary(geom="point", fun.y="mean", size=3., alpha=0.8) +

        xlab("Time (s)") +
        ylab("Percent error") +
        theme_light() + paper_theme_3  + guides(fill=FALSE)
...
data.resp$time_num <- as.numeric(as.character(data.resp$Time))
