...
data.resp <- data.resp.loc %>%
            ...
            filter((Dots_Counted < 150) & (abs_pct_err < 1))
...
ggplot(data=data.resp, aes(x=Time, y=100*abs_pct_err)) +
      stat_summary(geom="bar", alpha=0.8, fun.y="mean") +
      ...
      stat_summary(geom="point", fun.y="mean") +
        ...
        theme_light() + paper_theme_3 
ggsave("figs/Time_Abs_Pct_Err.png", width=5, height=4, dpi=400)
...
summary(lm(data=data.resp, pct_err ~ poly(Dots_Shown,2)  + log(Time_Num)))
summary(lm(data=data.resp, abs_pct_err ~ poly(Dots_Shown,2)  + log(Time_Num)))
...
ggplot(data.resp, aes(x=Dots_Shown, y=abs_pct_err, color=Time)) +
        ...
        stat_smooth(method="loess",span=1, se=TRUE,alpha=0.1) +

        stat_summary_bin(aes(x=Dots_Shown), binwidth=bw, geom="errorbar", 
                         fun.data="mean_se", position=position_dodge(width=1), size=1.7, width=0.25, alpha=0.9) +

        stat_summary_bin(aes(x=Dots_Shown), binwidth=bw,geom="point", fun.y="mean", position=position_dodge(width=1), size=2.5, alpha=1) +


        theme_light() + 
        xlim(10,81) + 
        xlab("Dots Shown") + ylab("Percent absolute error") +
        geom_hline(yintercept=0) +
        paper_theme_3  +
        scale_y_continuous(labels = scales::percent)  
ggsave("figs/Abs_Pct_Err.png", width=5,height=4)

bw <- 13
ggplot(data.resp, aes(x=Dots_Shown, y=pct_err, color=Time)) +
        stat_smooth(method="loess",span=1, se=FALSE) +
        stat_smooth(method="loess",span=1, se=TRUE,alpha=0.1) +
        ...
        theme_light() + 
        xlim(8,81) + 
        xlab("Dots Shown") + ylab("Percent error") +
        geom_hline(yintercept=0) +

        paper_theme_3 +
          #scale_x_log10(breaks=seq.int(0.6,1,0.2), labels=scales::percent, limits=c(0.58,1.02)) +
           # scale_y_log10(breaks=seq.int(-0.5,0.0,0.1), labels=scales::percent) 
          scale_y_continuous(labels = scales::percent)  
#scale_x_continuous(labels = scales::percent)  
ggsave("figs/Pct_Err.png", width=5,height=4)
...
ggplot(data.resp, aes(x=Dots_Shown, y=Dots_Counted)) +
       ...
       # coord_cartesian(xlim=c(10,40), ylim=c(0,80)) +
        ...
        ylab("Number of dots") + xlab("Dots shown") +
        #facet_wrap(~Time)  +
...
data.resp <- data.resp %>%
             ...
             mutate(mean_err_subj = mean(pct_err)*(time_num==0.1)) %>%
             group_by(Subject, pid) %>%  
             mutate(pos_neg=sign(sum(mean_err_subj))) %>%
             mutate(mean_err_subj=pos_neg * max(abs(mean_err_subj)))
                  

ggplot(data=data.resp, aes(x=Time, y=pct_err)) +
     # geom_violin(color="blue", alpha=0.75) +
      stat_summary(geom="line", alpha=0.5,size=0.25,
                   fun.y="mean", 
      ...
      stat_summary(geom="point", fun.y="mean",position=position_dodge(width=0.2), size=1., alpha=0.8, aes(group=Subject, color="Participant", shape="Participant"))+ 
        scale_y_continuous(labels = scales::percent) +
        stat_summary(geom="errorbar", width=0.4,size=1.0,aes(color="Group"), fun.data="mean_se") +
      stat_summary(geom="point", fun.y="mean", size=3., alpha=0.9, aes(color="Group", shape="Group")) +
       # coord_cartesian(ylim=c(-.6,.6)) +
        ...
        geom_hline(yintercept=0)
ggsave("figs/Time_Pct_Err.png", width=5, height=4, dpi=400)


data.resp <- data.resp %>%
          mutate(time_num=(as.numeric(as.character(Time)))) %>%
          group_by(pid, Subject) %>%
          mutate(z_score = (err - mean(err))/sd(err)) 

data.resp.bypart <- data.resp %>%
          group_by(pid, Subject, Time) %>%
          mutate(pct_err=median(pct_err)) %>%
          mutate(z_score=mean(z_score)) %>%
           top_n(n=1, wt=id) %>%
          group_by(Time) %>%
          mutate(mean_pct_err=mean(pct_err)) %>%
          mutate(median_pct_err=median(pct_err)) %>%
          mutate(sd_err= sd(mean_pct_err)) %>%
          mutate(time_s = paste(time_num, "s", sep=""))

ggplot(data=data.resp, aes(x=time_num, y=z_score)) +
     # geom_violin(color="blue", alpha=0.75) +
      #stat_summary(geom="bar", alpha=0.8, fun.y="mean") +
      stat_summary(geom="point", fun.y="mean",position=position_dodge(width=0.2), size=1., alpha=0.8, aes(group=Subject, color="Participant", shape="Participant"))+ 
       # scale_y_continuous(labels = scales::percent) +
  scale_x_log10(breaks=c(0.1,0.33,1,3), labels=c("0.1", "0.33", "1", "3")) + 
        stat_summary(geom="errorbar", width=0.2,size=1.0,alpha=0.9, fun.data="mean_se", aes(color="Group", shape="Group")) +
      stat_summary(geom="point", fun.y="mean", size=3., alpha=0.9, aes(color="Group", shape="Group")) +
       # coord_cartesian(ylim=c(-.6,.6)) +

        xlab("Time (s)") +
        ylab("Z-score of percent error") +
        theme_light() + paper_theme_3  + 
        theme(legend.text=element_text(size=10), legend.title=element_blank()) +
        guides(fill=FALSE) +
        scale_color_manual(name="a", values=c("Red", "Black"),labels=c("Group", "Participant")) +
...
ggsave("figs/Time_Pct_Err_Subj.png", width=5, height=4, dpi=400)

data.resp.bytime <- data.resp %>%
                  group_by(pid, Subject, Time) %>%
                  mutate(pct_err=mean(pct_err)) %>%
                  group_by(Time) %>%
                  mutate(sd_err = sd(pct_err))%>%
                  top_n(n=1, wt=trial_id)

table(data.resp.bytime$Time, data.resp.bytime$sd_err)
#table(data.resp.bypart$sd_err, data.resp.bypart$Time)


ggplot(data=data.resp, aes(x=Dots_Shown, y=Dots_Counted)) +
      stat_summary_bin(bins=35) + geom_abline() +
      theme_light() + paper_theme_3  +
      stat_smooth(method="lm", formula=y~x, size=0.8, se=FALSE) +
      stat_smooth(method="lm", formula=y~x, alpha=0.2, se=TRUE) +
      theme(axis.title.x=element_text(size=20), axis.title.y=element_text(size=20),axis.text.x=element_text(color="black")) +

      #stat_smooth(method="loess", span=2, size=0.8, se=FALSE) +
      #stat_smooth(method="loess", span=2, alpha=0.2,size=0, se=TRUE) +
      xlab("Number") + ylab("Estimate") +
      coord_cartesian(ylim=c(10,81), xlim=c(10,81))
    #  xlim(10,81) + ylim(10,81) 

ggsave("figs/estimate_number.png", width=5,height=5,dpi=400)
ggsave("figs/estimate_number.pdf", width=5,height=5,dpi=400)
...
ggplot(data=data.resp.binned, aes(x=Dots_Shown, y=sd_estimate)) +
            stat_summary_bin(bins=25, fun.data="mean_cl_boot") +
            theme_light() +  paper_theme_3 +
              stat_smooth(method="lm", formula=y~x, size=0.8, se=FALSE) +
              stat_smooth(method="lm", formula=y~x, alpha=0.2, se=TRUE) +
      theme(axis.title.x=element_text(size=20), axis.title.y=element_text(size=20),axis.text.x=element_text(color="black")) +

          xlab("Number") + ylab("Standard deviation of estimates")

ggsave("figs/sd_number.png", width=5,height=5,dpi=400)
ggsave("figs/sd_number.pdf", width=5,height=5,dpi=400)
...
```{r, fig.width=7.5, fig.height=5, results='asis', message=FALSE, echo=FALSE, warning=FALSE}
  ...
  data.resp.avs <- data.resp.loc %>%
                  ...
                  mutate(err=Dots_Counted - Dots_Shown) %>%
                  mutate(abs_err = abs(err)) %>%
                  #mutate(within_x = (gazeDist < 100)*0.65 + (gazeDist < 250) * 0.2 + 0.05) %>%
                 # mutate(within_x = 1 -  0.5**((belowX+.01)*0.5)) %>%
                  #mutate(within_x = (1-(1/(belowX+1.5))**0.5)) %>%
                  #mutate(within_x = 1-exp(-belowX)) %>%
                  #mutate(within_x =  (belowX > 3)) %>%
                  #mutate(within_x = pInGaze) %>%

                  #mutate(within_x = 2/(1 + exp(-1 -0.25*belowX)) - 1) %>%
          
                  group_by(trial_id, Subject, pid, Time) %>% 
                  mutate(belowX = mean(belowX)) %>%
                 # mutate(within_x = mean(within_x)) %>%

                  #mutate(within_x = mean(within_x)) %>%
                  mutate(within_x=mean(pInGaze)) %>%
                  #mutate(within_x = belowX) %>%
...
data.resp.avs$time_num <- as.numeric(as.character(data.resp.avs$Time))

```

```{r}

data.resp.avs <- data.resp.avs %>%

                  group_by(Dots_Shown) %>%
                  mutate(cent_err=(Dots_Counted - mean(Dots_Counted))) %>%
                  mutate(cent_abs_err=abs(cent_err)) %>%
                  #mutate(cent_abs_err=sd(cent_err)) %>%
                  mutate(sd_count=cent_abs_err/mean(Dots_Counted)) %>%
                  # mutate(sd_count=(Dots_Counted - mean(Dots_Counted))) %>%

                  #mutate(sd_count=sd_count) %>%
                  mutate(pct_dev = sd_count) 
                  #ungroup %>%
                  #mutate(pct_dev=sd(pct_dev))
                  # mutate(pct_dev=(pct_dev-mean(pct_dev))/sd(pct_dev))


#r.1 <- lmer(data=data.resp.avs, err ~ time_num + (1+time_num|Subject))
#r.2 <- lmer(data=data.resp.avs, err ~ time_num + within_x_tot + (1+time_num+within_x_tot|Subject))
summary(lm(data=data.resp.avs, err ~ seen_shown_diff + Dots_Shown))

lm(data=data.resp.avs, pct_err ~ Dots_Shown)
lm(data=data.resp.avs, pct_err ~ time_num)
lm(data=data.resp.avs, pct_err ~ time_num + Dots_Shown)
summary(lmer(data=data.resp.avs, pct_err ~ time_num + (1+time_num|Subject)))

summary(lm(data=data.resp.avs, cent_err ~ time_num + Dots_Shown))

summary(lm(data=data.resp.avs, cent_abs_err ~ time_num + Dots_Shown))
#summary(lmer(data=data.resp.avs.subj, pct_dev ~ Dots_Shown + time_num + (0+Dots_Shown+time_num|Subject)))
#summary(lmer(data=data.resp.avs.subj, pct_dev ~ time_num + Dots_Shown + (1+Dots_Shown|Subject)))


ggplot(data=data.resp.avs, aes(x=Time, y=pct_dev)) +
      stat_summary(fun.y="mean", geom="point", aes(group=Subject), alpha=0.5) +
      stat_summary(color="red") 


data.resp.avs.subj <- data.resp.avs %>%
                      group_by(Dots_Shown, Time) %>%
                      mutate(pct_dev=sd(Dots_Counted)/mean(Dots_Counted)) %>%
                      top_n(n=1, wt=id)
        
if(FALSE){
ggplot(data=data.resp.avs.subj, aes(x=Time, y=pct_dev)) +
     # geom_point() + 
      stat_summary(alpha=0.5,size=2.5, fun.y="mean", geom="point") +
      stat_smooth(method="lm", alpha=1, size=1.25, se=FALSE)  +
        scale_y_continuous(labels = scales::percent)  +
...
ggplot(data=data.resp.avs.subj, aes(x=Dots_Shown, y=pct_dev, color=Time)) +
      #geom_point() + 
      stat_smooth(method="lm", alpha=0.15, se=TRUE)  +
      xlim(10,80) + #coord_cartesian(ylim=c(0,0.6)) +
     # stat_summary(alpha=0.5,size=2.5, fun.y="mean", geom="point") +
      stat_smooth(method="lm", alpha=1, size=1.25, se=FALSE)  +

      stat_summary_bin(bins=25, alpha=0.8,size=2, fun.y="mean", geom="point") +
        scale_y_continuous(labels = scales::percent)  +
          ylab("Standard deviation") +
          theme_light() +  paper_theme_3  +  
    theme(legend.title=element_text(size=14), legend.text=element_text(size=12), 
      ...
      #scale_y_sqrt()
#stargazer(r.2)
ggsave("figs/dev_pd.png", width=5, height=4, dpi=300)

ggplot(data=data.resp.avs, aes(x=Dots_Shown, y=pct_err, group=Time, color=Time)) +

      stat_smooth(method="lm", alpha=0.15, se=TRUE)  +
      xlim(10,78) + 
     # coord_cartesian(ylim=c(-0.5,0.2)) +
     # stat_summary(alpha=0.5,size=2.5, fun.y="mean", geom="point") +
      #stat_summary_bin(bins=60, alpha=0.8,size=2.5, fun.y="mean", geom="point") +
      stat_summary_bin(bins=25, alpha=0.8,size=2., fun.y="mean", geom="point") +
...
ggsave("figs/err_pd.png", width=5, height=4, dpi=300)
        #+ scale_color_continuous(low="blue", high="red")

}
ggplot(data=data.resp.avs, aes(x=Dots_Counted %% 10)) +
      geom_histogram(binwidth=1, aes(y = ..density..)) +
          theme_light() + paper_theme_3 +
        xlab("Guess modulo 10") +       theme(strip.text=element_blank()) +
      scale_x_continuous(breaks=seq.int(0,9,1)) +
      ylab("Proportion of guesses") #+

     # facet_wrap(~Subject)
     # facet_wrap(~I(10 * round(Dots_Shown / 10)))


ggplot(aes(x=Dots_Counted %% 5), data=#data.resp.avs)+
       subset(data.resp.avs, data.resp.avs$Dots_Shown %% 5 != 0)) +
      geom_histogram(binwidth=1, aes(y = ..density..)) +
          theme_light() + paper_theme_3 +
        xlab("Guess modulo 5") +       theme(strip.text=element_blank()) +
      scale_x_continuous(breaks=seq.int(0,5,1)) +
      ylab("Proportion of guesses")



data.resp.avs.subs.3 <- data.resp.avs %>%
                        ungroup %>%
                        mutate(dc_mod_5 = Dots_Counted %% 5) %>%
                        filter(dc_mod_5 != 0) %>%
                        mutate(tot=n()) %>%
                        group_by(dc_mod_5) %>%
                        mutate(mod_5_pct= n()/tot) %>%
                        mutate(mod_5_dev = mod_5_pct * (1-mod_5_pct)/sqrt(26)) %>%
                        mutate(mod_5_max = mod_5_pct + mod_5_dev) %>%
                          mutate(mod_5_min= mod_5_pct - mod_5_dev) %>%

                        top_n(n=1,wt=trial_id)


ggplot(aes(x=dc_mod_5, y=mod_5_pct), data=data.resp.avs.subs.3)+
        geom_bar(alpha=0.7,stat='identity') +
        geom_point() +
      geom_errorbar(aes(ymin=mod_5_min, ymax = mod_5_max), width=0.0)+
          theme_light() + paper_theme_3 +
        xlab("Guess modulo 5") +      
...
ggplot(data.resp.avs, aes(x=within_x, y=pct_err, color=factor(Time))) +
        geom_point(aes(color=factor(Time)), alpha=0.2, size=2.) +
        #stat_summary_bin(bins=10, alpha=0.5, fun.y="mean", geom="point", size=3.) 
         # stat_summary_bin(bins=25,width=0, size=0.25, alpha=0.5, fun.data="mean_se", geom="errorbar") +
          stat_smooth(method="lm", se=TRUE,formula=y~poly(x,1),  aes(color=factor(Time)), span=5, size=2.25, alpha=0.2) +
  stat_smooth(method="lm", se=FALSE,formula=y~poly(x,1),  aes(color=factor(Time)), span=5, size=2.25, alpha=0.8) +
        xlab("Dots within gaze") +
        ylab("Error") +
          #xlim(0.01,1) +
        ...
        theme(legend.title=element_text(size=12), legend.text=element_text(size=10),
              legend.position=c(0.92,0.8), axis.title.x=element_text(face="plain", size=16, color="black"), axis.title.y=element_text(face="plain", size=16,color="black"),
              axis.text.y=element_text(face="plain", size=14,color="black"),
                            axis.text.x=element_text(face="plain", size=14,color="black")) +

          scale_y_continuous(labels = scales::percent, limits=c(-0.65,0.65)) +  #, breaks=c(-0.4,-0.3,-0.09,0,0.4)) +
    #geom_segment(angle=0, inherit.aes = FALSE, aes(x=0.1, y=-0.09, xend=0.99, yend=-0.09)) +
  # geom_segment(angle=0, inherit.aes = FALSE, aes(x=0.1, y=-0.3, xend=0.99, yend=-0.3)) +
    #geom_abline()
...
p.1 <- ggplot(data.resp.avs, aes(x=within_x, y=pct_err)) +
        #geom_point() +
        geom_point(aes(color=factor(Time)), alpha=0.25) +
        stat_smooth(method="loess", aes(color=factor(Time)), se=FALSE, span=1.25) +
...
p.3.1 <- ggplot(data.resp.avs, aes(x=within_x, y=abs_pct_err)) +
        ...
        #xlim(0,0.75) +
           scale_color_discrete(name="Time") +
        theme_light()  +paper_theme_3 +
scale_x_log10(breaks=seq.int(0.0,1,0.2))  
...
p.3
ggsave("figs/abs_err_time_seen.png", width=6,height=5,dpi=400)


 p.4 <- ggplot(data.resp.avs, aes(x=within_x, y=pct_err, color=factor(Time))) +
        geom_point(aes(color=factor(Time)), alpha=0.2, size=2.) +
        #stat_summary_bin(bins=7, alpha=0.5, fun.y="mean", geom="point", size=3.) +
         # stat_summary_bin(bins=7, alpha=0.5, fun.data="mean_se", geom="errorbar") +
  ...
  stat_smooth(method="lm", se=FALSE,formula=y~poly(x,1),  aes(color=factor(Time)), span=5, size=2.25, alpha=0.8) +
        ...
        theme_light() + 
        paper_theme_3 +
          scale_y_continuous(labels = scales::percent, limits=c(-0.7,0.3)) +
...
p.5.3.1 <- ggplot(data=data.resp.avs) +
        ...
        ylab("Error") + 
          ...
          scale_y_continuous(labels = scales::percent) +
         # scale_x_continuous(labels = scales::percent, limits=c(0.58,1.01))  +
           scale_x_continuous(labels = scales::percent, limits=c(-0.01,0.9))  +
        geom_hline(yintercept=0) +
        theme_light() +  paper_theme_3 

p.5.3.1
ggsave("figs/seen_by_error.png", width=5, height=5,dpi=400)
...
p.5.3.2 <- ggplot(data=data.resp.avs) +
        ...
        stat_summary(fun.data="mean_se", geom="errorbar", width=0.01,
          ...
          xlab("Dots seen") +
        ylab("Asolute error") +
         # scale_x_log10(breaks=seq.int(0.6,1,0.2), labels=scales::percent, limits=c(0.01,1.01)) +
          #  scale_y_log10(breaks=seq.int(0.0,0.5,0.1), labels=scales::percent) +
          scale_y_continuous(labels = scales::percent) +
        #  scale_x_continuous(labels = scales::percent, limits=c(0.58,1.01))  +
           scale_x_continuous(labels = scales::percent, limits=c(-0.01,0.9))  +
        geom_hline(yintercept=0) +
...
ggsave("figs/seen_by_abs_error.png", width=5, height=5,dpi=400)

p.5.3.3 <- ggplot(data=data.resp.avs, aes(x=Time, y=within_x)) +
        ...
        scale_shape_manual(name="a", values=c(19, 17), labels=c("Group", "Participant")) +
          ylab("Dots foveated") +
          theme_light() +  paper_theme_3  +  theme(legend.title=element_blank(), 
                                legend.text=element_text(size=12), legend.position=c(0.2, 0.85))

p.5.3.3

ggsave("figs/Time_by_seen.png", width=5, height=5,dpi=400)
ggsave("figs/Time_by_seen.pdf", width=5, height=5,dpi=400)
...
ggplot(data=data.resp.avs, aes(x=within_x_tot/(pctArea), y=Dots_Counted)) +
           ...
           #geom_point(size=0.5,alpha=0.5) +
            #coord_cartesian(ylim=c(8,163), xlim=c(8,163)) +
            stat_smooth(method='loess') +
            stat_summary_bin(binwidth=10, fun.y="mean", geom="point") +
            stat_summary_bin(binwidth=10, fun.data="mean_se", geom="errorbar") +
           # stat_summary_bin(binwidth=20, fun.y="mean", geom="line", aes(x=Dots_Shown, y=Dots_Counted), color="blue") +
          ...
          # ggtitle("Dots seen v. counted") + xlab("(dots seen) / (% area seen)") + ylab("Dots counted") +
            paper_theme_2  +
          #stat_summary(fun.y="mean", geom="bar", aes(x=bw*round((within_x_tot/pctArea)/bw, 2), y=err))  +  
          #stat_summary(fun.data="mean_se", geom="errorbar", aes(x=bw*round((within_x_tot/pctArea)/bw, 2), y=err)) +
          facet_wrap(~Time) + 
          xlab("Dots seen / area seen") + ylab("Human estimate") +
            geom_abline()# + 
           # xlim(0,175)


ggsave("figs/density_by_count.png", width=5,height=5)
...
ggplot(data=data.resp.avs, aes(x=within_x_tot, y=Dots_Counted)) +
          # geom_point(size=0.5,alpha=0.5) +
           ...
           # ggtitle("Dots seen v. counted") + xlab("Dots seen") + ylab("Dots counted") +
            paper_theme_2  +
            geom_abline() + 
           # coord_cartesian(ylim=c(8,78), xlim=c(8,78)) +
            stat_summary_bin(binwidth=5, fun.y="mean", geom="point") +
            stat_summary_bin(binwidth=5, fun.data="mean_se", geom="errorbar") +
...
ggsave("figs/accum_by_count.png", width=5,height=5)


ggplot(data=data.resp.avs, aes(x=pctArea*100, y=abs_pct_err*100, color=Time)) +
            #coord_cartesian(ylim=c(8,163), xlim=c(8,163)) +
            stat_smooth(method='lm', formula=y~x) +
           #stat_summary_bin(bins=10, fun.y="mean", geom="point") +
           # stat_summary_bin(bins=10, fun.data="mean_se", geom="errorbar") +
            paper_theme_2  +
         # facet_wrap(~Time) + 
          xlab("Percent of screen area seen") + ylab("Absolute percent error") +
            geom_hline(yintercept=0)
ggsave("figs/area_err.png", width=5, height=5)
#data.resp.avs$Time_Cond <- as.factor(round(as.numeric(as.character(data.resp.avs$Time,2))))


ggplot(data=data.resp.avs, aes(x=Time, y=pctArea*100)) +
    geom_jitter(alpha=0.1, width=0.1, height=0.04) + 

    stat_summary(fun.data="mean_se", color="red") +
       stat_summary(fun.y="mean", geom="point", color="red") +

       stat_summary(fun.y="mean", geom="line", color="red", aes(x=as.numeric(Time), y=pctArea*100)) +

    xlab("Time") + ylab("Percent of screen area seen") +
   # scale_x_log10()  +
    paper_theme_2

ggplot(data=data.resp.avs, aes(x=Time, y=within_x * 100)) +
        geom_jitter(alpha=0.1, size=1.25, width=0.1) +
        stat_summary(fun.data="mean_se", geom="errorbar", width=0.5) +

       stat_summary(fun.y="mean", geom="point", color="red") +
       stat_summary(fun.y="mean", geom="line", color="red", aes(x=as.numeric(Time), y=within_x*100)) +

        theme_light() + 
        paper_theme_3 + 
        ylab("Percent of dots seen")

ggplot(data=data.resp.avs, aes(x=pctArea, y=within_x,color=Time)) +
        geom_jitter(alpha=0.3, size=1.25, width=0.01, height=0.01) +

        stat_smooth(method="lm", formula=y~x)  +

        theme_light() + 
        paper_theme_3 + 
        ylab("Screen area seen") + xlab("Dots seen") +
        #  scale_x_log10(breaks=seq.int(0.2,1,0.2)*100) +
        #    scale_y_log10(breaks=seq.int(0.2,1,0.2)*100)  +
         # scale_y_continuous(labels = scales::percent) +
         #  scale_x_continuous(labels = scales::percent)  +
          scale_x_log10(breaks=seq.int(0.0,1,0.2), labels=scales::percent) +
            scale_y_log10(breaks=seq.int(0.0,1,0.2), labels=scales::percent) 

ggsave("figs/area_by_seen.png", width=5,height=5)

ggplot(data=data.resp.avs, aes(x=within_x_tot/(pctArea), y=within_x_tot, color=Time)) +

           geom_jitter(size=1.0,alpha=0.5, width=2, height=7) +
            geom_abline(alpha=0.9) +

            #stat_smooth(method='loess', span=1, size=1.2, se=FALSE) +
            stat_smooth(method='glm',formula=y~poly(x,2), size=1.2, se=FALSE) +

            ggtitle("(Dots seen) / (% area) v. dots seen") + xlab("Dots seen / (% area seen)") + ylab("Dots seen") +
            paper_theme_2  #+
           # ylim(7,140) + xlim(7,140)
    
#summary(lm(data=data.resp.avs, within_x ~ log(time_num)))
