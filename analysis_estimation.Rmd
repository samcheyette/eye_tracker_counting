...
ggplot(data=data.resp, aes(x=Time, y=pct_err)) +
     ...
     # stat_summary(geom="line", fun.y="median",position=position_dodge(width=0.2), size=.25, alpha=0.5, aes(group=Subject),linetype="dotted")+ 
        #geom_jitter(width=0.1,height=0.05, size=0.5,alpha=0.25) + 
        scale_y_continuous(labels = scales::percent) +
...
ggplot(data=data.resp, aes(x=Time, y= pct_err - mean_err_subj)) +
     # geom_violin(color="blue", alpha=0.75) +
      #stat_summary(geom="bar", alpha=0.8, fun.y="mean") +
      stat_summary(geom="point", fun.y="median",position=position_dodge(width=0.2), size=1., alpha=0.8, aes(group=Subject, color="Participant"), shape=2)+ 
        #geom_jitter(width=0.1,height=0.05, size=0.5,alpha=0.25) + 
        scale_y_continuous(labels = scales::percent) +
        stat_summary(geom="errorbar", width=0.25,size=1.0, fun.data="mean_se") +
      stat_summary(geom="point", fun.y="mean", size=3., alpha=0.8, aes(color="Group")) +
        #coord_cartesian(ylim=c(-.6,.6)) +

        xlab("Time (s)") +
        ylab("Percent error change from 0.1") +
        theme_light() + paper_theme_3  + guides(fill=FALSE, color=FALSE)
...
f_get_pred <- function(who) {
  who <- who[1]
  subs <- data.resp.cop %>%
          filter(Subject == who)
  
  r.1 <- loess(data=subs, Dots_Counted ~ time_num * Dots_Shown  )
  pred.1 <- predict(r.1, type="response", interval="prediction")
...
data.resp <- data.resp %>%
              group_by(Subject) %>%
              mutate(pred=f_get_pred(Subject)) %>%
              mutate(dev = abs(Dots_Counted - pred)) %>%
              filter(dev < 20) 
...
f_get_weber <- function(who, t) {
  who <- who[1]
  subs <- data.resp.cop %>%
          filter((Subject == who) & (time_num == t))
          #filter(Subject == who)

  r.2 <- glm(data=subs, dev ~ Dots_Shown - 1)
  
  return(r.2$coefficients["Dots_Shown"])
...
f_get_pred_dev <- function(who) {
  who <- who[1]
  subs <- data.resp.cop %>%
          filter(Subject == who)
  r.2 <- loess(data=subs, dev ~ Dots_Shown * time_num)
  pred.2 <- predict(r.2, type="response", interval="prediction")
...
data.resp <- data.resp %>%
              group_by(Subject) %>%
             mutate(pred_dev = f_get_pred_dev(Subject)) %>%
              group_by(Subject, Time) %>%
              mutate(weber=f_get_weber(Subject, time_num))
          

#r.1 <- loess(data=data.resp, Dots_Counted ~ time_num  * Dots_Shown  )

#pred.1 <- predict(r.1, type="response", interval="prediction")
#data.resp$pred_dots <- pred.1
#data.resp$dev <- abs(data.resp$Dots_Counted - data.resp$pred_dots)
#data.resp <- subset(data.resp, data.resp$dev < 25)
#r.2 <- glm(data=data.resp, dev ~ Dots_Shown * time_num)
#pred.2 <- predict(r.2, type="response", interval="prediction")
#data.resp$pred_dev <- pred.2
...
#ggplot(data=data.resp, aes(x=Dots_Shown, y=pred_dots, color=Time)) +
      #  stat_smooth(se=FALSE) 

#ggplot(data=data.resp.agg, aes(x=Time, y=dev)) +
    #    stat_summary(fun.data="mean_se") +
    #  stat_smooth(method="lm", se=TRUE)  

ggplot(data=data.resp, aes(x=dev)) +
       geom_histogram(bins=40)

ggplot(data=data.resp, aes(x=Dots_Shown, y=pred_dev, color=Time)) +
     #stat_summary_bin(fun.data="mean_se", bins=3) +
     stat_smooth(se=FALSE)

ggplot(data=data.resp, aes(x=Time, y=pred_dev))  +
      stat_summary(fun.data="mean_se") #+


data.resp.agg <- data.resp %>%
          group_by(Time, Subject) %>%
          top_n(n=1, wt=id)

ggplot(data=data.resp, aes(x=Time, y=weber))  +
      stat_summary(fun.data="mean_se")  #+
     # geom_point()
...
```{r, fig.width=7.5, fig.height=5, results='asis', message=FALSE, echo=FALSE, warning=FALSE}
  ...
  data.resp.avs <- data.resp.loc %>%
                 ...
                 # mutate(within_x = 1 -  0.5**((belowX+.01)*0.5)) %>%
                  ...
                  #mutate(within_x = 0.5*(belowX > 0) + 0.5 * (belowX > 4)) %>%
                  mutate(within_x = pInGaze**0.8) %>%
...
r.0 <- lm(data=data.resp.avs, abs_pct_err ~ Dots_Shown)
r.1 <- lm(data=data.resp.avs, abs_pct_err ~ time_num)
r.2 <- lm(data=data.resp.avs, abs_pct_err ~ time_num + poly(within_x, 1))
r.3 <- lm(data=data.resp.avs, abs_pct_err ~ time_num + poly(within_x, 2))
r.4 <- lm(data=data.resp.avs, abs_pct_err ~ within_x + Dots_Shown)
r.5 <- lm(data=data.resp.avs, abs_pct_err ~  within_x + time_num + Dots_Shown)

#r.1 <- lm(data=data.resp.avs, Dots_Counted ~ Dots_Shown)
#r.2 <- lm(data=data.resp.avs, Dots_Counted ~ Dots_Shown + within_x_tot )

#r.1 <- lm(data=data.resp.avs, pct_err ~ time_num)
#r.2 <- lm(data=data.resp.avs, pct_err ~ time_num + within_x)
...
p.5.3.3 <- ggplot(data=data.resp.avs, aes(x=Time, y=within_x)) +
          stat_summary(aes(group=Subject, color="Participant", shape="Participant"),alpha=0.7, position=position_jitter(width=0.04,height=0.05), fun.y="median", geom="point") +
          ...
          stat_summary(fun.data="mean_cl_boot", geom="errorbar", width=0.3, size=1) +
          scale_y_continuous(labels = scales::percent)  +
        scale_color_manual(name="a", values=c("Red", "Black"),labels=c("Group", "Participant")) +
        scale_shape_manual(name="a", values=c(19, 17), labels=c("Group", "Participant")) +
          ylab("Dots foveated") +
          theme_light() +  paper_theme_3  +  theme(legend.title=element_blank(), legend.text=element_text(size=12))

p.5.3.3

ggsave("figs/Time_by_seen.png", width=5, height=4,dpi=400)
