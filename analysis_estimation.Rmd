...
p.3 <- ggplot(data.resp.avs, aes(x=within_x, y=abs_pct_err)) +
        ...
        stat_smooth(method="loess", se=FALSE, aes(color=factor(Time)), span=2, size=1.5) +
        xlab("P(dots within gaze)") +
...
p.5 <- ggplot(data.resp.avs, aes(x=abs_seen_shown_diff, y=abs_err)) +
      #  geom_point(aes(color=factor(Time)), alpha=0.35, size=2.) +
        ...
        stat_smooth(method="loess", se=FALSE, aes(color=factor(Time)), span=1, size=1.5) +
...
p.x <- ggplot(data.resp.avs, aes(x=factor(pid), y=mean_dist)) +
      stat_summary(fun.data="mean_cl_boot")
    

p.x <- ggplot(data.resp.avs, aes(x=factor(pid), y=pct_err)) +
      stat_summary(fun.data="mean_cl_boot")

p.4 <- ggplot(data.resp.avs, aes(x=belowX, y=pct_err)) +
        geom_point(aes(color=factor(Time))) +
        stat_smooth(method="lm", se=FALSE, aes(color=factor(Time)), span=2) +
        xlab("Mean time in window per dot") +
        ylab("Absolute percent error")+
          scale_x_log10()+
        geom_hline(yintercept=0)
    

p.5 <- ggplot(data.resp.avs, aes(x=belowX, y=pct_err, color=factor(Time))) +
        geom_point() +
        
        stat_smooth(method="lm", se=FALSE, aes(color=factor(Time)), span=1.5)  +
...
p.6 <- ggplot(data.resp.avs, aes(x=belowX, y=pct_err)) +
        geom_point() +
